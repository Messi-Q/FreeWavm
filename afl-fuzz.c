...
enum {
  ...
  /* 04 */ PY_FUNC_TRIM,
  /* 05 */ PY_FUNC_FEED,   // Information feedback
  PY_FUNC_COUNT
};
...
static int init_py() {
  ...
  if (module_name) {
    ...
    if (py_module != NULL) {
      ...
      py_functions[PY_FUNC_FEED] = PyObject_GetAttrString(py_module, "get_info_feedback");  // add
...
static void fuzz_py(char* buf, size_t buflen, char* add_buf, size_t add_buflen, char** ret, size_t* retlen) {
  ...
  if (py_module != NULL) {
    ...
    if (py_value != NULL) {
      ...
      *ret = malloc(*retlen);
      // printf("memcpy dest %llx,src %llx,size:%x\n",*ret,PyByteArray_AsString(py_value),*retlen);
...
// static u32 get_cov_py() {

//   PyObject *py_args, *py_value;

//   py_value = PyObject_CallObject(py_functions[PY_FUNC_COV], py_args);
...
// If you want achieve the hybridization of two test cases, you can remove the annotations
//retry_external_pick:
  ...
  //  target = queue;
   ...
   // }
    ...
    fuzz_py(out_buf, len, "", 0, &retbuf, &retlen);
